...
use crate::snake::domain::square::Square::{EmptySquare, FoodSquare};

#[derive(Debug)]
pub struct Board {
    length: usize,
    width: usize,
    squares: Vec<Vec<Rc<RefCell<Square>>>>,
}
...
impl Board {
    pub fn new(length: usize, width: usize) -> Self {
        Self {
            ...
            width,
            squares: (0..length)
                .map(|_| (0..width).map(|_| Rc::new(RefCell::new(EmptySquare))).collect())
                .collect(),
        }
    }

    pub fn get_middle_square(&self) -> Rc<RefCell<Square>> {
        self.squares[self.width / 2][self.length / 2].clone()
    }

    pub fn get_adjacent_square(&self, square: &Rc<RefCell<Square>>, direction: &Direction) -> Option<Rc<RefCell<Square>>> {
        let mut coord = None;
    ...
    pub fn spawn_food(&self) -> Result<(), GameOverEvent> {
        match self.squares.iter()
            .flatten()
            .filter(|&s| *s.as_ref().borrow() == EmptySquare)
            .choose(&mut rand::thread_rng()) {
            Some(s) => {
                *s.borrow_mut() = FoodSquare;
                Ok(())
            }
